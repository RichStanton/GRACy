...
def greedyElongation(seq):
	...
	while True:
		...
		os.system(installationDirectory+"src/conda/bin/blastn -query unmapped.fasta -db toElong.fasta -outfmt 6 -num_threads 8 -dust no -soft_masking false -out outputBlast.txt  >/dev/null 2>&1")
		#print "Done"
...
while True:
	...
	print("Aligning.....")
	os.system(installationDirectory+"src/conda/bin/bowtie2 --local --very-fast-local -k 3 -p "+numThreads+"  -x toElong -1 "+ read1 + " -2 " + read2 + " -S "+ projectName+".sam >/dev/null 2>&1" )

	os.system(installationDirectory+"src/conda/bin/samtools view -bS -F 8 -f 4 "+ projectName+".sam > unmapped.bam 2>/dev/null")
	os.system(installationDirectory+"src/conda/bin/bedtools bamtofastq -i unmapped.bam -fq unmapped.fastq >/dev/null 2>&1")
	os.system(installationDirectory+"src/conda/bin/fastq_to_fasta -i unmapped.fastq -o unmapped.fasta -Q33 >/dev/null 2>&1")
	...
	os.system(installationDirectory+"src/conda/bin/cap3 unmapped.fasta > cap3Assembly 2>/dev/null")
